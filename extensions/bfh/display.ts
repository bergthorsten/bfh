...
export function stateToolText(statePath: string, state: HarnessState): string {
  return [
    ...
    `Revision: ${state.revisionCount}/${state.revisionLimit} (human loops: ${state.humanRevisionCount})`,
    `Review: ${state.review.verdict} (${formatReviewCountsLine(state.review)})`,
    `Design: ${designReviewStatusLabel(state)}`,
    `Human: pre-implement=${state.human.preImplement.status}${state.human.preImplement.required ? " (required)" : ""}, post-review=${state.human.postReview.status}, pre-close=${state.human.preClose.status}`,
